...
class GenerateAssignmentReportPDF {
  ...
  Future<void> generateReport(
    ...
    await saveAndLaunchFile(bytes, 'Assignment.pdf');
  }
...
//Create PDF grid and return
  PdfGrid getGrid(
    ...
    headerRow.cells[0].value = 'Student Name';
    ...
    headerRow.cells[1].stringFormat.textDirection =
        PdfTextDirection.rightToLeft;
    headerRow.cells[2].value = 'Grade';
    headerRow.cells[2].stringFormat.alignment = PdfTextAlignment.center;
    headerRow.cells[2].stringFormat.textDirection =
        PdfTextDirection.rightToLeft;

    void addStudent(
        String studentName, String status, String grade, PdfGrid grid) {
      final PdfGridRow row = grid.rows.add();
      row.cells[0].value = studentName;
      row.cells[1].value = status;
      row.cells[2].value = grade;
    }

    //Add rows
    for (var i in submittedList) {
      addStudent(i.name, i.submissionState, i.grade, grid);
    }

    for (var i in notSubmittedList) {
      addStudent(i.name, 'Not Submitted', '-', grid);
    }
