...
import '../../course_members/model/course_member_model.dart';
import 'save_file_mobile.dart' if (dart.library.html) 'save_file_web.dart';

class GenerateCourseMembersReportPDF {
  ...
  Future<void> generateReport(
    List<Groupmember> enrolledList,
  ) async {
    late File file;
    file = File('/storage/emulated/0/Download/ report course_members.pdf');
    ...
    final PdfLayoutResult result =
        drawHeader(page, pageSize, grid, 'Course members', font);
    //Draw grid
    ...
    await saveAndLaunchFile(bytes, 'Course qqmembers.pdf');
  }
...
//Create PDF grid and return
  PdfGrid getGrid(
    ...
    headerRow.cells[2].stringFormat.textDirection =
        PdfTextDirection.rightToLeft;
    headerRow.cells[3].value = 'School Name';
    ...
    void addStudent(String studentId, String studentIdName, String centerName,
        String schoolName, PdfGrid grid) {
      final PdfGridRow row = grid.rows.add();
      row.cells[0].value = studentId;
      row.cells[1].value = studentIdName;
      row.cells[2].value = centerName;
      row.cells[3].value = schoolName;
    }

    //Add rows
    for (var i in enrolledList) {
      addStudent(i.id, i.fullName, i.centerName, i.schoolName, grid);
    }
